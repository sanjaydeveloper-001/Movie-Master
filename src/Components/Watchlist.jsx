    ...
    <div className="flex flex-col w-full h-max items-center">
      ...
      <div className="flex flex-col h-[88vh] w-full p-20 gap-10">
          ...
          <div className="flex gap-x-5 text-[18px] w-full border-b-1 border-amber-400">
            {tabs.map((tab)=> {
              return (
                <h1 onClick={()=>setActiveBar(tab.id)} className={`flex items-center cursor-pointer gap-x-1 ${activeBar == tab.id ? 'border-b-4 border-yellow-400' : 'border-none'}`}>
                  {tab.icon}
                  {tab.label}
                </h1>
              )
          ...
          {
            watchList.length == 0 ? <div className="text-xl "> 
              No WatchList
            </div> : 

          <div className="flex flex-wrap gap-10 pb-15"> 
            {
              activeBar == 1 ? watchList.map((movie)=> {
                return ( <WatchCard movie={movie} toWatch={toWatch} watching={watching} watched={watched} activeBar={activeBar} HandleAddToWatch={HandleAddToWatch} />)
              }) : 
              ( activeBar == 2 ? 
                toWatch.map((movie)=> {
                return ( <WatchCard movie={movie} toWatch={toWatch} watching={watching} watched={watched} activeBar={activeBar} HandleAddWatching = {HandleAddWatching}  />)
              }) : 
              ( activeBar == 3 ? 
                watching.map((movie)=> {
                return ( <WatchCard movie={movie} toWatch={toWatch} watching={watching} watched={watched} activeBar={activeBar} HandleAddWatched = {HandleAddWatched} />)
              }) : 
                watched.map((movie)=> {
                return ( <WatchCard movie={movie} toWatch={toWatch} watching={watching} watched={watched} activeBar={activeBar} HandleRemoveWatched = {HandleRemoveWatched} />)
              })
              )
              )
            }
